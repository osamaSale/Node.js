...
const createProduct = async (req, res) => {
    ...
    if (name === "") {
        ...
        image = req.file
        if (image = req.file) {
            image = await cloudinary.uploader.upload(req.file.path, { folder: "Store-Mobile/products" });
        ...
        connection.query(sql, async (err, result) => {
            ...
            if (err) {
                let public_id = data.cloudinary_id === null ? "null" : data.cloudinary_id.replace('Store-Mobile/products/g', '')
...
const editProduct = async (req, res) => {
    ...
    connection.query(sql, async (err, result) => {
        ...
        if (err) {
            ...
            if (image) {
                let public_id = product.cloudinary_id.replace('Store-Mobile/products/g', '')
